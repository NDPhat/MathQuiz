...
class AnimatedBounce extends StatefulWidget {
  const AnimatedBounce({Key? key}) : super(key: key);

  @override
  _AnimatedBounceState createState() => _AnimatedBounceState();
}
          ...
          builder: (context, child) {
            if (!isClick) {
              if (ball.yVel != 0 || isClickAfter) {
                ...
                ball.subYpos(
                    0.5 * accel * pow(baseTime, 2) - ball.yVel * baseTime);
...
class myBall {
  ...
  myBall.origin() {
    ...
    for (double i = 0; i < ballRad - 1; i++) {
      draw.addOval(Rect.fromCircle(center: Offset(xPos, yPos), radius: i));
    }
  }
